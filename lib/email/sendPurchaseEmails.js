...
const mailgun = connectToMailgun({apiKey: process.env.MAILGUN_KEY, domain: 'mail.hintsy.io'})

function sendPurchaseEmails (order, db) {
  let botToken = db.ref('/slack_accounts/' + order.metadata.team_id + '/bot/bot_access_token')
  botToken.once('value').then(function (snapshot) {
    ...
    (async function () {
      ...
      // Add purchaser to our email list.
      addUserToEmailList(purchaserData.data.user.profile.email, purchaserData.data.user.real_name, purchaserData.data.user.team_id, purchaserData.data.user.id)
    })().catch(function (err) {
      // Error handler
