...
function sendPurchaseEmails (order, db) {
  ...
  teamAccountToken.once('value').then(function (snapshot) {
    ...
    (async function () {
      ...
      // Get the purchaser's data, and send them the receipt email
      const purchaserData = await getUserDataFromUserID(accessToken, purchaserID)
      sendReceiptEmail(purchaserData.data.user.real_name, purchaserData.data.user.profile.email, order, mailgun)
    })().catch(function (err) {
      // TODO: Handle error
      console.log(err)
    })
