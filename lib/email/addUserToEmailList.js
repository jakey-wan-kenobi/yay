...
function addUserToEmailList (email, name, teamID, userID, isFromWebsite) {
  ...
  const newUser = {
    ...
    vars: {
      team_id: teamID,
      user_id: userID,
      from_website: isFromWebsite
    }
  }
  list.members().create(newUser, function (err, data) {
    if (err) {
      captureException(err + '. Details: ' + ' ' + email + ' ' + name + ' ' + teamID + ' ' + userID + ' from website? ' + isFromWebsite, 'Error saving new user to mailing list.', 297958)
      return false
    }
    return true
  })
