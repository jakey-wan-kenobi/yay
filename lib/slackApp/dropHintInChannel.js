...
function dropHintInChannel (channelID, teamID, timestamp) {
  (async function () {
    ...
    const authTokenPromise = getSlackTokenFromTeamID(teamID)
    ...
    let message = _composeMessage(botToken, channelID, false)
    _postMessage(message).then(function (response) {
      if (!response.data.ok && response.data.error === 'channel_not_found') {
        // SECOND TRY. Try one more time, this time as user. NOTE: It seems I need the chat:write:user permission scope for this function call alone.
        message = _composeMessage(authToken, channelID, true)
        _postMessage(message).then(function (res) {
          if (!res.data.ok) {
            captureException(res.data, 'Response.ok is false from Slack while dropping hint in Slack channel.', 389273)
            return
          }
        })
      } else if (!response.data.ok && response.data.error !== 'channel_not_found') {
        // This is an error which we haven't expected. We should capture it.
...
function _composeMessage (token, channelID, postAsUser) {
  return qs.stringify({
    'token': token,
    'channel': channelID,
    'as_user': postAsUser,
