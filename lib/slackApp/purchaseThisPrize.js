...
function purchaseThisPrize (callbackID, teamID, purchaser, db) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    let accounts = db.ref('/slack_accounts_users/' + teamID + '/' + purchaser.id)
    let stripeID = ''
    accounts.once('value').then(function (snapshot) {
      stripeID = snapshot.child('stripe_id').val()
      if (!stripeID) {
      ...
      const recipientHandle = getUserNameFromHandle(callbackID)
