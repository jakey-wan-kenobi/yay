...
import getUserNameFromUserID from '../account/getUserNameFromUserID'
...
function setupAuthRoute (server) {
  ...
  server.get('/auth', function (req, res) {
    ...
    async function _performSetup () {
      ...
      // Fetch user's name
      if (!userRealName) {
        const fetchedUser = await getUserNameFromUserID(result.data.access_token, result.data.user_id)
    ...
    _performSetup().catch(function (err) {
