...
function setupAuthRoute (server) {
  ...
  server.get('/auth', function (req, res) {
    ...
    (async function () {
      ...
      // Fetch user's name
      if (!userRealName) {
        // NOTE: If we use the bot's access token, then we don't need the users:read permission scope to access the users.list method. The bot has access to that method already! Woo!
        const fetchedUser = await getUserDataFromUserID(result.data.bot.bot_access_token, result.data.user_id)
