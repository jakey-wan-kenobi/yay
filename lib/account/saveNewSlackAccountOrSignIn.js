...
function saveNewSlackAccountOrSignIn (body, db, res) {
  let response = new Promise(function (resolve, reject) {
    ...
    accounts.once('value').then(function (snapshot) {
      ...
      // NOTE: TEAM EXISTS. In this case, we never add more things to the database here. Just sign them in.
      ...
      // NOTE: TEAM DOES NOT EXIST YET.
      // Check that the data we have is for an INSTALL and not a sign in. We should not save initial team data if it's a sign in only. If it's just for a sign in and they haven't yet installed, redirect them to a page which prompts them to add to slack or go back to Yay. We can do this by checking for 'bot' object on the data, which is present on installs only.
      if (!body.bot) {
        res.redirect('https://yay.hintsy.io/err/bot_not_installed')
        return
      }
