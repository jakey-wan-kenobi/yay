...
import sendSuccessMessage from '../slackApp/sendSuccessMessage'
    ...
    switch (data.actions[0].name) {
      case 'did_choose_prize':
        ...
        purchaseThisPrize(data.callback_id, data.team.id, data.user, db).then(function (val) {
          sendSuccessMessage(undefined, data.channel.id, data.team.id, res)
          ...
          if (err === 'missing_credit_card') {
            ...
            // Track Heap event
            heap.track('Purchase Attempt', heapUserID, {
      ...
      case 'cancel':
        const message = {
          'text': 'Okay, we\'ll figure it out later. 😘',
          'attachments': [{
            'fallback': 'Can\'t show image.',
            'image_url': 'https://media.giphy.com/media/l4KibK3JwaVo0CjDO/giphy.gif'
          }]
        }
        res.send(message)
        // Track Heap event
      ...
      case 'yes_drop_hint':
        ...
        sendSuccessMessage(true, null, null, res)
      ...
      case 'no_drop_hint':
        ...
        sendSuccessMessage(false, null, null, res)
        // Track Heap event
