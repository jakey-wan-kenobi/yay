      ...
      case 'did_choose_prize':
        ...
        purchaseThisPrize(data.callback_id, data.team.id, data.user, db).then(function (val) {
          ...
          if (err === 'missing_credit_card') {
            ...
            captureException(err, 'User attempted purchase without an available credit card', 315097)
            return
          }
          ...
          if (err.code === 'out_of_inventory') {
            ...
            captureException(err, 'User attemped purchase of a product with 0 inventory.', 357864)
            return
          }
