...
function setupYayMessageButtons (server) {
  server.post('/yay-message-buttons', function (req, res) {
    ...
    if (data.actions[0].name === 'did_choose_prize') {
    ...
    } else if (data.actions[0].name === 'yes_intro') {
      ...
      res.send(response)
    } else if (data.actions[0].name === 'no_intro') {
      // Update the original message to remove the intro buttons
      const response = getIntroMessage(undefined, undefined, false)
      res.send(response)
    } else if (data.actions[0].name === 'yes_drop_hint') {
      const response = getCelebrationMessage(true)
      res.send(response)
    } else if (data.actions[0].name === 'no_drop_hint') {
      const response = getCelebrationMessage(false)
      res.send(response)
    }
    // res.send('yes')
  })
