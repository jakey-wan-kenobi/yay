...
function setupYaySlashCommands (server) {
  ...
  server.post('/yay', function (req, res) {
    ...
    if (typeof recipientHandle !== 'string') {
      ...
      res.send(message)
      raven.captureMessage('User attempted slash command to purchase a prize without including a real @handle person.')
      return
    }
    ...
    if (recipientHandle === '@channel' || recipientHandle === '@here' || recipientHandle === '@everyone') {
      ...
      res.send(message)
      raven.captureMessage('User attempted slash command for @here, @channel, or @everyone.')
      return
    }
    ...
    (async function () {
      ...
      if (!realFriend) {
        ...
        res.send(message)
        raven.captureMessage('User attempted slash command for a real @handle, but one that is not a real person on their team.')
        return
      }
      if (realFriend === 'isBot') {
        res.send('That\'s very nice, but bots don\'t have shipping addresses (or corporeal bodies). Try a human.')
        raven.captureMessage('User attempted slash command to purchase prize for a bot.')
        return
      }
      ...
      console.log('STILL DEBUGGING -- When response times out, check if we are alerted to it here? If this is false, it may have timed out?', res.headersSent)
    })().catch(function (err) {
      // Error handler
