...
function setupYaySlashCommands (server) {
  ...
  server.post('/yay', function (req, res) {
    ...
    if (typeof recipientHandle !== 'string') {
      ...
      res.send('Hm. I couldn\'t find that user. Make sure to use their Slack username, like *@stanley*!')
      return
    }
    ...
    (async function () {
      ...
      if (!realFriend) {
        res.send('Hm. I couldn\'t find *' + recipientHandle + '* on your team. Are you sure you typed it right?')
        return
      }
      if (realFriend === 'isBot') {
